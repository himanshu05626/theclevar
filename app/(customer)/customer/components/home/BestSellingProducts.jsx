...
export default function BestSellingProducts({ products, customerId }) {
  ...
  const handleRemoveFromCart = (product) => {
    ...
    startTransition(async () => {
    console.log('deleteCartItem2',customerId)

      // LOGGED-IN USER
      if (customerId) {
    console.log('deleteCartItem3')
      ...
      <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-4">
        {products.map((product, index) => {
          ...
          return (
            <div
              key={product.id}
              className="rounded-xl bg-[#1a1a1a] shadow-lg border border-white/10 transition hover:shadow-2xl"
            >
              {/* IMAGE */}
